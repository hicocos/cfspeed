...
- Web 运行间隔使用小时（h），默认 6 h，允许对应整数秒的小数；加载时秒除以 3600，保存时乘以 3600。API/TOML 仍使用 interval_seconds 整数秒（30–604800），不改变既有配置兼容性。
- GET /api/admin/config → {revision:int,service:{source_url,interval_seconds,timeout_seconds,attempts,dry_run,max_ips,pushplus_token_env},targets:[Target fields],credentials:[{name:string,configured:bool}]}
