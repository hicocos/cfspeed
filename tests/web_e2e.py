...
with sync_playwright() as p:
    ...
    check('overview recent run opens shared detail modal')
    ...
    source_interval = page.get_by_label(re.compile(r'^IP 获取间隔（min）'))
    expect(source_interval).to_have_value('360')
    ...
    expect(source_interval).to_have_value('360')
    source_interval.fill('30')
    with page.expect_request(lambda r: r.method=='PATCH' and r.url.endswith('/api/admin/config')) as request:
        page.get_by_role('button',name='保存设置',exact=True).click()
    assert request.value.post_data_json['service']['source_interval_seconds'] == 1800
    assert 'interval_seconds' not in request.value.post_data_json['service']
    expect(page.get_by_text('设置已保存并生效。',exact=True)).to_be_visible()
    page.reload()
    expect(source_interval).to_have_value('30')
    expect(interval).to_have_value('1.5')
    ...
    # Isolated server has no scheduler loop; install a dummy target only for config validation.
    result=page.evaluate("""async()=>{const a=await(await fetch('/api/auth/session')).json();const c=await(await fetch('/api/admin/config')).json();const r=await fetch('/api/admin/config',{method:'PATCH',headers:{'Content-Type':'application/json','X-CSRF-Token':a.csrf},body:JSON.stringify({revision:c.revision,targets:[{label:'isolated config only',provider:'cloudflare',name:'cf.example.com',zone_id:'a'.repeat(32),token_env:'E2E_CONFIG_TOKEN'}],secrets:{E2E_CONFIG_TOKEN:'dummy-not-a-real-token'}})});return r.status}""")
    assert result==200
    page.reload()
    page.get_by_label('运行模式').select_option('false')
    page.get_by_role('button',name='保存设置',exact=True).click()
    page.get_by_placeholder('启用同步',exact=True).fill('启用同步')
    page.get_by_role('button',name='确认启用',exact=True).click()
    expect(page.get_by_text('设置已保存并生效。',exact=True)).to_be_visible()
    source_interval.fill('15')
    with page.expect_request(lambda r:r.method=='PATCH' and r.url.endswith('/api/admin/config')) as request:
        page.get_by_role('button',name='保存设置',exact=True).click()
    assert request.value.post_data_json['service']=={'source_interval_seconds':900}
    assert 'confirm_apply' not in request.value.post_data_json
    expect(page.get_by_role('dialog')).to_have_count(0)
    expect(page.get_by_text('设置已保存并生效。',exact=True)).to_be_visible()
    page.reload()
    expect(source_interval).to_have_value('15')
    expect(page.get_by_label('运行模式')).to_have_value('false')
    page.goto(base+'/admin/stats')
    preview=page.get_by_role('button',name='立即预览',exact=True)
    sync=page.get_by_role('button',name='立即同步',exact=True)
    expect(sync).to_be_visible()
    style='el=>{const s=getComputedStyle(el);return [s.backgroundColor,s.color,s.border,s.borderRadius,s.padding,s.fontSize]}'
    assert preview.evaluate(style)==sync.evaluate(style)
    page.goto(base+'/admin/settings')
    page.get_by_label('优选来源').select_option('https://ip.v2too.top/api/nodes')
    expect(page.get_by_label('运行模式')).to_have_value('true')
    page.get_by_label('运行模式').select_option('false')
    page.get_by_role('button',name='保存设置',exact=True).click()
    expect(page.get_by_role('button',name='确认启用',exact=True)).to_be_disabled()
    page.get_by_role('button',name='取消',exact=True).click()
    page.reload()
    page.get_by_label('运行模式').select_option('true')
    page.get_by_role('button',name='保存设置',exact=True).click()
    expect(page.get_by_text('设置已保存并生效。',exact=True)).to_be_visible()
    result=page.evaluate("""async()=>{const a=await(await fetch('/api/auth/session')).json();const c=await(await fetch('/api/admin/config')).json();const r=await fetch('/api/admin/config',{method:'PATCH',headers:{'Content-Type':'application/json','X-CSRF-Token':a.csrf},body:JSON.stringify({revision:c.revision,targets:[],secrets:{E2E_CONFIG_TOKEN:null}})});return r.status}""")
    assert result==200
    check('live interval-only save needs no confirmation, persists mode; scope change still requires confirmation; preview/sync computed styles match')
