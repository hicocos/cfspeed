...
- **独立周期**：分别设置 IP 获取和 DNS 同步间隔，互不绑定。
- **网页管理**：维护凭据、修改管理员密码。
...
首次部署无需填写 DNS 凭据，稍后在网页中添加。IP 获取和 DNS 同步默认各每 6 小时运行，启动先获取再同步；初始为预览模式，不修改 DNS。旧配置未设置 `source_interval_seconds` 时继承原 `interval_seconds`。
...
1. 在「服务设置」选择 IP 来源，分别设置 IP 获取间隔和 DNS 同步间隔，保持预览模式并保存。
2. 在「总览与统计」点击「立即预览」。
